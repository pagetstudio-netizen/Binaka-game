...
export default function Register() {
  ...
  const InputWrap = ({ children, error }: { children: React.ReactNode; error?: string }) => (
    <div>
  ...
  const iconStyle = { color: "rgba(255,255,255,0.40)" };
  const inputClass = "flex-1 h-full px-3 bg-transparent text-sm outline-none text-white placeholder:text-white/30";
  const labelClass = "text-xs font-black uppercase tracking-wide mb-1.5 block";
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col select-none" style={{ background: BG }}>
      ...
      <div className="w-full relative flex-shrink-0" style={{ maxHeight: 220 }}>
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 30%, #071C12 100%)" }} />
      </div>

      {/* ── FORM ── */}
      <div className="flex-1 -mt-6 rounded-t-3xl z-10 flex flex-col px-5 pt-5 pb-10" style={{ background: BG }}>

        {/* TABS */}
        <div className="flex rounded-2xl p-1 mb-5" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
          <Link href="/login" className="flex-1 py-2.5 rounded-xl text-center text-sm font-bold"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <InputWrap error={errors.fullName?.message}>
              <User size={16} className="ml-4 shrink-0" style={iconStyle} />
              <input type="text" placeholder="Votre nom et prénom" className={inputClass} {...form.register("fullName")} />
            </InputWrap>
          </div>

          {/* Pays + Téléphone */}
          <div>
            <label className={labelClass} style={labelStyle}>Pays & Numéro de téléphone</label>
            <div className="flex items-center h-13 rounded-2xl overflow-hidden transition-colors"
              style={{ background: INPUT_BG, border: `1.5px solid ${BORDER}` }}>
              <Phone size={16} className="ml-4 shrink-0" style={iconStyle} />
              <div className="relative flex items-center h-full shrink-0" style={{ borderRight: `1px solid ${BORDER}` }}>
                <select className="h-full pl-2 pr-7 bg-transparent text-xs font-bold outline-none appearance-none cursor-pointer text-white"
                ...
                <ChevronDown size={11} className="absolute right-1 pointer-events-none" style={iconStyle} />
              </div>
              <input type="tel" inputMode="numeric" placeholder="Numéro de téléphone" className={inputClass} {...form.register("phone")} />
            </div>
          ...
          <div>
            ...
            <InputWrap error={errors.password?.message}>
              <Lock size={16} className="ml-4 shrink-0" style={iconStyle} />
              <input type={showPassword ? "text" : "password"} placeholder="Min. 6 caractères" className={inputClass} {...form.register("password")} />
              <button type="button" className="px-4" style={iconStyle} onClick={() => setShowPassword(!showPassword)}>
          ...
          <div>
            ...
            <InputWrap error={errors.confirmPassword?.message}>
              <Lock size={16} className="ml-4 shrink-0" style={iconStyle} />
              <input type={showConfirm ? "text" : "password"} placeholder="Retapez le mot de passe" className={inputClass} {...form.register("confirmPassword")} />
              <button type="button" className="px-4" style={iconStyle} onClick={() => setShowConfirm(!showConfirm)}>
          ...
          {/* Code promo */}
          <div>
            <label className={labelClass} style={labelStyle}>Code promo (optionnel)</label>
            <InputWrap>
              <Gift size={16} className="ml-4 shrink-0" style={iconStyle} />
              <input type="text" placeholder="Code de parrainage" className={inputClass} {...form.register("referralCode")} />
            </InputWrap>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-xs flex-1 leading-snug" style={{ color: "rgba(255,255,255,0.35)" }}>Cliquez sur 🔄 pour un nouveau code</p>
            </div>
            <div className="flex items-center h-13 rounded-2xl overflow-hidden transition-colors"
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="text-xs leading-snug" style={{ color: "rgba(255,255,255,0.50)" }}>
              J'ai lu et j'accepte les{" "}
              <span className="font-bold underline" style={{ color: GREEN }}>conditions d'utilisation</span>{" "}
              de BINAKA GAME. Je certifie avoir plus de 18 ans.
            </span>
          </label>
          {errors.terms && <p className="text-red-400 text-xs -mt-1 pl-1">{errors.terms.message}</p>}

          {/* CRÉER MON COMPTE */}
          <motion.button type="button" whileTap={{ scale: 0.97 }}
            onClick={form.handleSubmit(onSubmit)}
            disabled={registerMutation.isPending || !termsAccepted}
            className="w-full h-14 rounded-2xl text-white font-black text-base tracking-wide disabled:opacity-50 flex items-center justify-center gap-2"
            style={{ background: termsAccepted ? "linear-gradient(135deg,#0F8A5F,#0A5C3A)" : "rgba(255,255,255,0.12)", boxShadow: termsAccepted ? "0 4px 20px rgba(15,138,95,0.4)" : "none" }}>
          ...
          <p className="text-center text-[11px] pt-1" style={{ color: "rgba(255,255,255,0.30)" }}>🔒 Inscription sécurisée · BINAKA GAME v1.0.0</p>
        </div>
      </div>
